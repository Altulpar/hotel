...
async function main() {
  ...
  if (!hotel) {
    await prisma.hotelInfo.create({
      data: {
        hotelName: "Ad Otelcilik Fiore Gökçeada",
        slogan: "Denize, doğaya ve sakin ada hayatına açılan otel.",
        aboutText:
          "Ad Otelcilik Fiore Gökçeada; sade tasarımı, ferah odaları ve yerel dokuyu hissettiren atmosferiyle misafirlerine huzurlu bir kaçış sunar. Sabahları taze kahvaltı kokusu, gün içinde deniz esintisi ve akşamları sakin teras sohbetleri buradaki ritmin parçasıdır.",
        locationText:
    ...
    await prisma.hotelInfo.update({
      ...
      data: {
        ...
        aboutText: hotel.aboutText
          .replaceAll("Ada Ruhu Otel", "Ad Otelcilik Fiore Gökçeada")
          .replaceAll("Fiore Otel", "Ad Otelcilik Fiore Gökçeada")
      }
    });
  }
